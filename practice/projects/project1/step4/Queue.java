...
public class Queue{
  ...
  /** This method filters out all tweets that contain the keyword supplied
   * @param keyword The String to compare tweets against
   */
  ...
  /** This method returns a Queue with all tweets that occurred on the input
   * date
   * @param date The format for the date is : year-month-day i.e. 2014-1-28
   */
  public Queue filterForDate(String date){
    Queue retval = new Queue();
    Node temp = this.head;
    do{
      if(temp.data.onDate(date))
        retval.enqueue(temp.data);
      temp = temp.next;
    }while(temp != null);
    return retval;
